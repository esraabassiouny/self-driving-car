...
def send_motor(left_pwm, right_pwm):
    cmd = f"L{left_pwm:03d}R{right_pwm:03d}\n"
...
video_name = f"lane_following_{time.strftime('%Y%m%d_%H%M%S')}.avi"
...
try:
    while True:
        ...
        # Start U-turn only if cooldown has passed
        if lane_end_detected and state == STATE_FOLLOW_LANE and (current_time - last_uturn_time) > 8.0:
        ...
        # STOP 2
        # ---------------------------------------
        if state == STATE_STOP2:

            send_motor(0, 0)

            if current_time - state_start_time > 0.5:
                state = STATE_UTURN
                state_start_time = current_time
        ...
        # U-TURN
        # ---------------------------------------
        if state == STATE_UTURN:

            # Perform a left U-turn: left wheel backward, right wheel forward
            send_motor(0, 180)
            # Rotate for a fixed duration to complete the turn
            turn_duration = 1.2  # seconds, adjust as needed
            turn_start = time.time()
            while time.time() - turn_start < turn_duration:
                time.sleep(0.05)
            # Stop motors after turn
            send_motor(0, 0)
            # Record completion time and resume lane following
            last_uturn_time = time.time()
            state = STATE_FOLLOW_LANE
            continue
